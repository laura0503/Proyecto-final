  ...
  Widget build(BuildContext context) {
    ...
    for (final tramo in tramos) {
      ...
      for (final sesion in sesiones) {
        final profSesion = profesores
            .firstWhereOrNull((p) => p.nombre == sesion.profesor);
        final profId =
    ...
    final hasContent = slots.isNotEmpty;

    return SingleChildScrollView(
      physics: const AlwaysScrollableScrollPhysics(),
      padding: EdgeInsets.fromLTRB(context.horizontalPadding, 12, context.horizontalPadding, 32),
      child: hasContent
          ? Column(
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        ...cards,
        if (cards.isNotEmpty)
          _AddButton(tramo: tramo, dia: dia, onTap: onEmptySlotClick)
        else
          _EmptySlot(tramo: tramo, dia: dia, onTap: onEmptySlotClick),
      ],
    );
  }
}
...
class _AddButton extends StatelessWidget {
  ...
  const _AddButton(
      {required this.tramo, required this.dia, required this.onTap});
