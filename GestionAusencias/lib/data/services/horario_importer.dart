...
class HorarioImporter {
  ...
  /// Sincroniza TODOS los departamentos de la base de datos basándose en las asignaturas reales
  Future<void> sincronizarTodo() async {
    print("***** Iniciando Sincronización PROFUNDA de Departamentos *****");
    try {
      ...
      final Map<int, String> profNames = {};
      final Map<int, String?> profDepts = {};

      for (final h in horarios) {
        final id = h['id_profesor'] as int;
        final asignatura = h['Asignaturas']['nombre'] as String;
        final deptoActual = h['profesores']['departamento'] as String?;
        profNames[id] = h['profesores']['nombre'] as String;
        profDepts[id] = deptoActual;
      ...
      for (final id in profSubjects.keys) {
        if (profDepts[id] == null || profDepts[id] == "General" || profDepts[id] == "Pendiente") {
          ...
          if (deptoDeducido != null) {
            print("Sincronizando Prof: ${profNames[id]} -> $deptoDeducido");
      ...
      print("Error en sincronización profunda: $e");
    }
  }
  ...
  Future<void> subirASupabase(String csvContent) async {
    ...
    if (row0.toLowerCase() == "id" || row0.toLowerCase().contains("id_")) return;
    ...
    if (records.isNotEmpty) {
      await _importarRelacionesASupabase(records);
    }
  }
        ...
        for (int diaIndice = 1; diaIndice <= 5; diaIndice++) {
          ...
          for (int l = 1; l < lines.length; l++) {
            ...
            if (line.startsWith('(') && line.endsWith(')')) fAula = line.substring(1, line.length - 1);
              ...
              for (final g in line.split(',')) if (g.trim().isNotEmpty) fGrupos.add(g.trim());
            }
          }
    ...
    for (final record in records) {
      try {
        final tramoId = await _getOrCreateId('horario_tramo', 'id_horario', {
          'texto': record.tramoTexto,
          'horario_inicio': record.horarioInicio ?? "08:00",
          'horario_fin': record.horarioFin ?? "09:00",
          'es_guardia': record.asignaturaNombre.toUpperCase().contains("GUARDIA"),
          'recreo': record.tramoTexto.toLowerCase().contains("recreo")
        });
