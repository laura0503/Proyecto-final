...
def normalize_text(text):
    ...
    # Limpieza agresiva de caracteres no imprimibles o rotos
...
def get_id_fuzzy(table, column, value):
    ...
    # Si falla, imprimir debug
    print(f"DEBUG: No se encontro match para '{value}' (norm: '{val_norm}') en {table}.{column}")
    return None
...
def clean_csv_content(content):
    # Intentar varios patrones para la cabecera
    patterns = [r';Lunes;Martes;M', r';LUNES;MARTES;M', r';Lunes;Martes;']
    match_start = None
    for p in patterns:
        match_start = re.search(p, content, re.IGNORECASE)
...
def procesar_un_archivo(ruta_archivo):
    ...
    try:
        ...
        if not id_profe: return

        # Borrar registros previos para este profesor
        print(f"Borrando registros previos para ID {id_profe}...")
